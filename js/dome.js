...
function convertImgToBuffer() {
	...
	buffer.width = original_img.width;
	buffer.height = original_img.height;

	buffer_ctx.drawImage(original_img, 0, 0);

	var buffer_imgdata = buffer_ctx.getImageData(0, 0, buffer.width, buffer.height);
	var buffer_pixels = buffer_imgdata.data;

    original_img_buffer = new Array(original_img.width * original_img.height * 3);
    for (var i = 0, j = 0; i < buffer_pixels.length; i += 4, j += 3) {
        original_img_buffer[j] = buffer_pixels[i];
        original_img_buffer[j + 1] = buffer_pixels[i + 1];
        original_img_buffer[j + 2] = buffer_pixels[i + 2];
    }
}

function dewarp() {
	if (original_img_buffer != null) {
        ...
        for (var i = 0; i < equi_image_height; i++) {
            for (var j = 0; j < equi_image_width; j++) {
				...
				equi_pixels[dest_offset] = original_img_buffer[src_offset];
				equi_pixels[dest_offset + 1] = original_img_buffer[src_offset + 1];
				equi_pixels[dest_offset + 2] = original_img_buffer[src_offset + 2];
